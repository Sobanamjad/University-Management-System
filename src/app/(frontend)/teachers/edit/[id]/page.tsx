  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (userRes.id) {
          if (userRes.role !== 'teacher' && userRes.role !== 'coordinator') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.errors?.[0]?.message || 'Failed to update teacher')
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-gray-500 font-bold tracking-widest uppercase text-xs">Fetching Teacher Records...</p>
      </div>
    )
  }
        ...
        <div className="max-w-4xl mx-auto px-6 w-full flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Teacher ID: {id}</p>
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {formData.role === 'coordinator' && (
                      <div className="space-y-4 md:col-span-2">
                        <label className="text-sm font-bold text-gray-700 ml-1">Assigned Departments</label>
