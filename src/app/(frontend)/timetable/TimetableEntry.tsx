...
export default function TimetableEntry({ entry, viewType }: TimetableEntryProps) {
  ...
  return (
    <div
      className={`group relative h-full w-full p-3 rounded-xl border transition-all duration-200 hover:shadow-lg hover:scale-[1.02] bg-white ${entry.status === 'cancelled' ? 'opacity-60 grayscale' : ''}`}
    >
      <div className="flex items-center justify-between mb-2 gap-2">
        <span
          className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full border ${getStatusColor(entry.status)}`}
        >
          {entry.status}
        </span>
        <span
          className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full ${getLectureTypeColor(entry.lectureType || 'theory')}`}
        >
      ...
      <div className="space-y-1.5">
        {viewType === 'class' ? (
          <div className="flex items-center text-xs text-gray-600">
            <User
              size={12}
              className="mr-1.5 text-gray-400 group-hover:text-blue-500 transition-colors"
            />
