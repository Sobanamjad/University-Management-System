// src/app/(frontend)/universities/create/page.tsx
'use client'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.errors?.[0]?.message || 'Failed to create university')
      }
    } catch (err) {
      setError('An error occurred while creating the university.')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="px-6 w-full flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Register a new university into the system</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-2xl bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mt-6">
          <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="mt-8 pt-6 border-t border-gray-100 flex justify-end space-x-4">
              ...
              >
                <Save size={18} />
                <span>{loading ? 'Creating...' : 'Save University'}</span>
              </button>
            </div>
          </form>
        </div>
